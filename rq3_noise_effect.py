"""
RQ3: Evaluate the effect of using noise in the diffusion process. 
"""
...
def get_args():
    ...
    parser.add_argument("--expr_path", default="results/rq3", help="Name of experiment save dir.")
    
    args = parser.parse_args()

    return args

def eval_lsb_sanitize(covers, secrets, diff_model, path):
    """
    Evaluate suds and dm-suds with containers made with lsb for various dmsuds timesteps without added noise.
    """
    #
    # Get lsb containers
    #
    containers, reveal_secret = use_lsb(covers, secrets) # returned [0, 1]
    save_steg_imgs(covers[N:M], secrets[N:M], containers[N:M], reveal_secret[N:M], path)
    # get metrics
    ...
    for t in t_s:
        chat_diff = diff_model.sanitize(containers, t=t, add_noise=False)
        ...
        # 
        if t < 525 or t > 975:
            save_path = os.path.join(path, "lsb", f"step_{t}")
    ...
    df.to_csv(os.path.join(path, 'metrics_lsb.csv'), index=False)
        

def eval_ddh_sanitize(covers, secrets, diff_model, HnetD, RnetD, path):
    """
    Evaluate suds and dm-suds with containers made with ddh for various dmsuds timesteps without added noise.
    """
    #
    # Get ddh containers
    #
    ...
    for t in t_s:
        ...
        # 
        if t < 525 or t > 975:
            save_path = os.path.join(path, "ddh", f"step_{t}")
            save_step_imgs(chat_diff[N:M], reveal_chat_diff[N:M], save_path)
        else:
            print(f"timestep {t} ..............")
    ...
    df.to_csv(os.path.join(path, 'metrics_ddh.csv'), index=False)


def eval_udh_sanitize(covers, secrets, diff_model, Hnet, Rnet, path):
    """
    Evaluate suds and dm-suds with containers made with udh for various dmsuds timesteps without added noise.
    """
    #
    # Get udh containers
    #
    ...
    for t in t_s:
        ...
        # 
        if t < 525 or t > 975:
            save_path = os.path.join(path, "udh", f"step_{t}")
            save_step_imgs(chat_diff[N:M], reveal_chat_diff[N:M], save_path)
        else:
            print(f"timestep {t} ..............")
    ...
    df.to_csv(os.path.join(path, 'metrics_udh.csv'), index=False)
...
def main():
    ...
    #
    eval_lsb_sanitize(covers, secrets, diff_model, args.expr_path)
    eval_ddh_sanitize(covers, secrets, diff_model, HnetD, RnetD, args.expr_path)
    eval_udh_sanitize(covers, secrets, diff_model, Hnet, Rnet, args.expr_path)
