"""
Evaluate DM-SUDS on imagenet with PRIS
"""
...
def get_args():
    ...
    parser.add_argument("--save_dir", type=str, default="results/rq4_pris")
...
def main():
    ...
    for i, covers in enumerate(tqdm.tqdm(data_loader)):
        ...
        print(containers.max(), containers.min())
        #
        # Sanitize
        #
        print("\n\nSanitizing ...")
    ...
    with open(os.path.join(args.save_dir, 'imagenet_pris_table.txt'), 'w') as file:
        file.write(table)
